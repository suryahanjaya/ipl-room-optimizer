...
# =========================================================
# PRIORITIZE PACKING INTO 3 DUMMY ROOMS, CORRECT CAMPUS:
# - H6-GD only in CS2 (ROOM EXAM CAPACITY = 250)
# - NTD   only in CS2 (ROOM EXAM CAPACITY = 300)
# - B5-GD only in CS1 (ROOM EXAM CAPACITY = 130)
#
# Approach:
# - For each KEY (KEY_CA), only add dummy rooms matching that KEY's CAMPUS (COSO).
...
NEW_ROOMS = [
    {"ROOM ID": "NTD",   "ROOM EXAM CAPACITY": 300, "CAMPUS": "CS2"},
...
def main():
    ...
    for key_val in key_list:
        ...
        for r in NEW_ROOMS:
            ...
            row["SUC_CHUA"] = int(r["ROOM EXAM CAPACITY"])  # treat as ROOM EXAM CAPACITY
    ...
    for key_val, group in df.groupby("KEY_CA", dropna=False):
        ...
        # dummy first, then larger classes
        ...
        # detailed output (exclude dummy courses from COURSES MERGED)
        for b in bins:
            course_list = ", ".join(
                [
        ...
        rooms_after = len(bins)  # includes dummy bins even if empty
