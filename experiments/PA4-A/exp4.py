...
#
# Input 1) students_by_course_datetime_campus.csv
#   Required columns: "COURSE ID", "DATE_ONLY", "TIME", "CS1", "CS2"
#
# Input 2) rooms_capacity_campus.csv
#   Required columns: "ROOM ID", "CAPACITY", "EXAM CAPACITY", "CAMPUS_NORM"
#   Optional column: "CAMPUS_CONFLICT" (TRUE/FALSE) -> if TRUE, the room is excluded
#
...
#   - Maintain remaining students per course
#   - For each room, iterate courses by remaining desc:
...
def split_allocate_fill_rooms(
    ...
    for r in rooms_sorted:
        room_id = r["room_id"]
        room_cap = int(r["cap"])            # total room capacity (all courses combined)
        room_exam_cap = int(r["exam_cap"])  # max per (course, room)
        ...
        for cid in sorted_courses():
            ...
            # chunk respects:
            #  - remaining course demand
            #  - remaining room total capacity
            #  - per-(course,room) exam capacity
...
def build_overall_summary(
    ...
    # Weighted utilization over USED room-slots
