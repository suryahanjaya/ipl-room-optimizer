...
#      columns: COURSE ID, DATE_ONLY, TIME, CS1, CS2 (TOTAL optional)
#   2) rooms_capacity_campus.csv
#      columns: ROOM ID, EXAM CAPACITY, CAMPUS_NORM (CS1/CS2) (+ CAPACITY optional)
#
# - Output per (DATE_ONLY, TIME, CAMPUS):
#   * allocation_plan.csv
#   * allocation_room_summary.csv
#   * allocation_unassigned.csv
#   * rooms_used_per_day_summary.csv
#   * tongket.csv  <-- NEW: overall summary + key stats printed
# =========================================================
...
def load_students(students_path: str) -> pd.DataFrame:
    ...
    # ensure DATE_ONLY is comparable (string ok); keep as-is
    return df
...
def load_rooms(rooms_path: str) -> pd.DataFrame:
    ...
    df["CAMPUS_NORM"] = df["CAMPUS_NORM"].astype(str)
    return df
...
def split_allocate_fill_rooms(
    ...
    remaining = {d["course_id"]: int(d["students"]) for d in demands}
    meta = {d["course_id"]: d for d in demands}
    ...
    rooms_sorted = sorted(rooms, key=lambda r: int(r["exam_cap"]), reverse=True)
    ...
    for r in rooms_sorted:
        ...
        for cid in sorted_courses():
            ...
            # per-(course,room) chunk <= room_exam_cap, and <= remaining room cap
...
def build_overall_summary(
    ...
    if rooms_used_pivot is not None and not rooms_used_pivot.empty:
        # columns: DATE_ONLY, CS1_SUM, CS2_SUM, TOTAL_ROOMS_USED_SUM, CS1_PEAK, ...
        total_room_times = int(rooms_used_pivot["TOTAL_ROOMS_USED_SUM"].sum())
        ...
        total_room_times = 0
        total_room_times_cs1 = 0
        total_room_times_cs2 = 0
        peak_concurrent = 0

    # Utilization (weighted by room capacities)
    if alloc_out is not None and not alloc_out.empty:
        # capacity sum per used room instance (slot+campus+room) then compute weighted utilization
        room_sum = (
...
def main():
    ...
    for (date_only, time_val), slot in students_df.groupby(["DATE_ONLY", "TIME"]):
        for campus in ["CS1", "CS2"]:
            ...
            rooms = [{"room_id": r["ROOM ID"], "exam_cap": r["EXAM CAPACITY"]} for r in rooms_by_campus[campus]]
    ...
    if not alloc_out.empty:
        ...
        room_sum = pd.DataFrame(
            columns=["DATE_ONLY", "TIME", "CAMPUS", "ROOM ID", "ROOM_EXAM_CAPACITY", "TOTAL_STUDENTS", "NUM_MODULES", "UTILIZATION"]
        )
