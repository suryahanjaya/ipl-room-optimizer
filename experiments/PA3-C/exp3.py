...
# + ADD 3 DUMMY ROOMS (AVAILABLE FOR ALL SLOTS), CORRECT CAMPUS
# + PRIORITIZE DUMMY ROOMS FIRST
#
# DUMMY ROOMS:
...
#
# "Priority" mechanism:
#   - Put dummy rooms first in rooms_sorted
#   - Keep sorting by exam_cap desc within dummy and within others
# =========================================================
...
DUMMY_ROOMS = [
    {"ROOM ID": "NTD", "EXAM CAPACITY": 300, "CAMPUS_NORM": "CS2"},
...
def load_rooms(rooms_path: str) -> pd.DataFrame:
    ...
    df["ROOM ID"] = df["ROOM ID"].astype(str)
    df["CAMPUS_NORM"] = df["CAMPUS_NORM"].apply(norm_campus)

    # Optional CAPACITY column: keep if exists, but not required in this algorithm
    if "CAPACITY" in df.columns:
        df["CAPACITY"] = pd.to_numeric(df["CAPACITY"], errors="coerce")

    # ---- Add / overwrite dummy rooms into inventory (available for all slots) ----
    dummy_df = pd.DataFrame(DUMMY_ROOMS)
    dummy_df["ROOM ID"] = dummy_df["ROOM ID"].astype(str)
    ...
    df = df[~df["ROOM ID"].isin(dummy_df["ROOM ID"])].copy()
...
def split_allocate_fill_rooms(
    ...
    dummy_room_ids: dummy rooms to prioritize (already campus-filtered)

    Returns:
      allocations: chunk allocations (splitting allowed)
      unassigned: leftover demand
    """
    ...
    for r in rooms_sorted:
        ...
        for cid in sorted_courses():
            ...
            chunk = min(rem_students, room_remaining_cap)  # exam cap already equals room capacity here
            if chunk <= 0:
                continue
...
def build_overall_summary(
    ...
    return pd.DataFrame([{
        ...
        "DUMMY_ROOMS_ADDED": "NTD@CS2(300); H6-GD@CS2(250); B5-GD@CS1(130)",
...
def main():
    ...
    if not alloc_out.empty:
        ...
        room_sum = pd.DataFrame(
            columns=["DATE_ONLY", "TIME", "CAMPUS", "ROOM ID", "ROOM EXAM CAPACITY", "TOTAL STUDENTS", "NUM_MODULES", "UTILIZATION"]
        )
